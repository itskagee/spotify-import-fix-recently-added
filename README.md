...
This script fixes the random listing of items in playlists that happens when songs are imported to Spotify using services like TuneMyMusic or Soundiiz and sorted by 'Recently Added'.

## Note
It was built and tested on Windows, but should run fine on other systems as well.
...
It does this by first scanning the existing playlist, then adding the songs to a new playlist one by one in reverse order with a 1 second delay in between. While this makes the process a bit long for huge playlists, it is the only way we can be assured that the final sorted list works as intended.

The script saves its current working state whenever there is any kind of error. So if it crashes, hits an API limit, your internet dies, or whatever, just re-run it (make sure to note the error it shows though). 

It does not modify your existing playlist, and makes a note of the new one it creates, so this re-run is deterministic and idempotent.

For playlists up to 700 items long, the script works in a single go. For playlists longer than 750-800 items, you may run into Spotify's daily API limits. Just wait 24h (or however long the script tells you), and start it back up to continue from where you left off.
...
- Get the Spotify Client ID and Secret for your app:
    ...
    - Add `http://127.0.0.1:8080/callback` as the Redirect URI in your app settings.
        ...
        - To check the availability of the port, open your 'Terminal' and enter these commands (they can take a while to run, be patient):
            - Windows (elevated / administrator terminal): `netstat -a | findstr 8080` 
...
- (Optional / Advanced) Install one of the following on your system:
    - Docker
        - Can be either Docker Desktop or Docker CE.
    - Golang
...
- Clone the repository, or download/copy the `src` folder, the `docker` folder, `go.mod`, and `go.sum` into a new folder.
...
### Using Golang
- Clone the repository, or download/copy the `src` folder, `go.mod`, and `go.sum` into a new folder.
